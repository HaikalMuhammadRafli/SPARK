...
<div class="p-4">
    <div class="flex items-center p-4 mb-4 text-red-800 border border-red-300 rounded-lg bg-red-50" role="alert">
        ...
        <div>
            ...
            <p class="font-normal text-xs">Apakah Anda yakin ingin menghapus data kompetensi ini?</p>
        </div>
    </div>

    <div class="grid grid-cols-2 rounded-lg">
        <div class="text-center border border-gray-200 py-2 rounded-tl-lg">
            <h6 class="text-sm text-gray-600">ID</h6>
        </div>
        <div class="text-center border border-gray-200 py-2 rounded-tr-lg">
            <p class="text-sm text-gray-600">{{ $kompetensi->kompetensi_id }}</p>
        </div>
    </div>
    <div class="grid grid-cols-2 rounded-lg">
        <div class="text-center border border-gray-200 py-2 rounded-bl-lg">
            <h6 class="text-sm text-gray-600">Nama Kompetensi</h6>
        </div>
        <div class="text-center border border-gray-200 py-2 rounded-br-lg">
            <p class="text-sm text-gray-600">{{ $kompetensi->kompetensi_nama }}</p>
        </div>
    </div>

    <div class="mt-4">
        <form id="form" data-kompetensi_id="{{ $kompetensi->kompetensi_id }}">
    ...
    $(document).ready(function() {
        $("#form").validate({
            submitHandler: function(form, event) {
                ...
                const kompetensi_id = $(form).data('kompetensi_id');
                ...
                $.ajax({
                    url: `/api/kompetensi/${kompetensi_id}`,
                    method: 'DELETE',
                    headers: {
