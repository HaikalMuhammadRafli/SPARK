    ...
    <section class="bg-white flex flex-col md:flex-row md:justify-between md:items-center gap-4 p-4 rounded-xl mb-2">
        ...
        <div class="flex flex-col md:flex-row gap-2 w-full md:w-auto">
            <div class="flex flex-row gap-2 flex-wrap">
                <x-buttons.table-actions type="button" title="Export PDF" color="primary" :onclick="'window.location.href=\'' . route('admin.manajemen-pengguna.admin.export-pdf') . '\''"  />
            ...
            <div class="flex flex-row gap-2 flex-wrap">
                ...
                <x-buttons.table-actions type="button" title="Tambah Baru" color="primary"
                    onclick="modalAction('{{ route('admin.manajemen-pengguna.admin.create') }}')" />
            </div>
        </div>
    </section>
...
@push('js')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const route = 'http://127.0.0.1:8000/api/admin';
...
<td class="px-4 py-1 text-left">
    <div class="inline-flex rounded-md shadow-xs" role="group">
        <button type="button"
            onclick="openEditModal('${item.nip}')"
        ...
        <button type="button"
            onclick="openDeleteModal('${item.nip}')"
...
`;
        ...
        function openEditModal(id) {
            const url = `{{ url('admin/manajemen-pengguna/admin') }}/${id}/edit`;
            console.log(url);
            modalAction(url);
        }

        function openDeleteModal(id) {
            const url = `{{ url('admin/manajemen-pengguna/admin') }}/${id}/delete`;
            modalAction(url)
        }
    </script>
@endpush
