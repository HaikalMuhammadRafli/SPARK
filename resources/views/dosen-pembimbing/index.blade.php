        ...
        <div class="flex flex-col md:flex-row gap-2 w-full md:w-auto">
            ...
            <div class="flex flex-row gap-2 flex-wrap">
                ...
                <x-buttons.table-actions type="button" title="Tambah Baru" color="primary"
                    onclick="modalAction('{{ route('admin.manajemen-pengguna.dosen-pembimbing.create') }}')" />
            </div>
        </div>
    </section>
...
@push('js')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const route = 'http://127.0.0.1:8000/api/dosen-pembimbing';
...
`;
        ...
        function openEditModal(id) {
            const url = `{{ url('admin/manajemen-pengguna/dosen-pembimbing') }}/${id}/edit`;
            console.log(url);
            modalAction(url);
        }

        function openDeleteModal(id) {
            const url = `{{ url('admin/manajemen-pengguna/dosen-pembimbing') }}/${id}/delete`;
            modalAction(url)
        }
    </script>
@endpush
