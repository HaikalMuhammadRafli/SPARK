...
@section('content')
        ...
        <div class="flex flex-col md:flex-row gap-2 w-full md:w-auto">
            ...
            <div class="flex flex-row gap-2 flex-wrap">
                ...
                <x-buttons.default type="button" title="Tambah Baru" color="primary"
                    onclick="modalAction('{{ route('admin.master.kompetensi.create') }}')" />
            </div>
        </div>
    </section>
    ...
    <section class="overflow-x-auto bg-white px-4 py-4 rounded-xl">
        ...
        <x-data-table id="table-body" :headers="[
            ['title' => 'ID', 'key' => 'kompetensi_id', 'sortable' => true],
            ['title' => 'Nama Kompetensi', 'key' => 'kompetensi_nama', 'sortable' => false],
...
@push('js')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const route = 'http://127.0.0.1:8000/api/kompetensi';
            ...
            fetch(route, {
                ...
                .then(data => {
                    ...
                    data.data.forEach((item, index) => {
                        ...
                        row.innerHTML = `
<td class="px-4 py-1">${item.kompetensi_id}</td>
<td class="px-4 py-1">${item.kompetensi_nama}</td>
<td class="px-4 py-1 text-left">
    <div class="inline-flex rounded-md shadow-xs" role="group">
        <button type="button"
            onclick="openEditModal('${item.kompetensi_id}')"
        ...
        <button type="button"
            onclick="openDeleteModal('${item.kompetensi_id}')"
...
`;
        ...
        function openEditModal(id) {
            const url = `{{ url('admin/master/kompetensi') }}/${id}/edit`;
            console.log(url);
            modalAction(url);
        }

        function openDeleteModal(id) {
            const url = `{{ url('admin/master/kompetensi') }}/${id}/delete`;
            modalAction(url)
        }
    </script>
@endpush
