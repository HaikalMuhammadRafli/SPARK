...
@section('content')
    <section class="bg-white flex flex-col md:flex-row md:justify-between md:items-center gap-4 p-4 rounded-xl mb-2">
        ...
        <div class="flex flex-row gap-2">
            <x-buttons.table-actions type="button" title="Kembali" color="secondary" 
                onclick="window.location.href='{{ route('admin.manajemen.lomba.index') }}'" />
            <x-buttons.table-actions type="button" title="Edit" color="primary" 
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <section class="bg-white p-4 rounded-xl">
            ...
            @if($lomba->lomba_poster_url)
                <div class="w-full">
                    <img src="{{ asset('storage/' . $lomba->lomba_poster_url) }}" 
        ...
        <section class="lg:col-span-2 bg-white p-4 rounded-xl">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <p class="text-gray-900">{{ $lomba->lomba_mulai_pendaftaran->format('d F Y') }}</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Akhir Pendaftaran</label>
                    <p class="text-gray-900">{{ $lomba->lomba_akhir_pendaftaran->format('d F Y') }}</p>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div class="bg-green-50 rounded-lg p-3">
                            ...
                            <p class="text-sm font-semibold text-green-900">{{ $lomba->lomba_mulai_pelaksanaan->format('d/m/Y') }}</p>
                        </div>
                        @endif
                        ...
                        <div class="bg-red-50 rounded-lg p-3">
                            ...
                            <p class="text-sm font-semibold text-red-900">{{ $lomba->lomba_selesai_pelaksanaan->format('d/m/Y') }}</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="md:col-span-2 mt-4 pt-4 border-t border-gray-200">
                    <div class="flex flex-col sm:flex-row gap-4 text-xs text-gray-500">
                        <p><span class="font-medium">Dibuat:</span> {{ $lomba->created_at->format('d F Y H:i') }}</p>
                        <p><span class="font-medium">Diperbarui:</span> {{ $lomba->updated_at->format('d F Y H:i') }}</p>
                        @if($lomba->validated_at)
                        <p><span class="font-medium">Divalidasi:</span> {{ $lomba->validated_at->format('d F Y H:i') }}</p>
                        @endif
                    </div>
                </div>
            </div>
